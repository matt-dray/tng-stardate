...
#
tng_ep_names <- tng_ep_wiki %>%
  rvest::html_nodes(".wikiepisodetable tr > :nth-child(3)") %>%
  rvest::html_text() %>% 
  dplyr::tibble() %>% 
  rename(episode_title = ".") %>% 
  filter(episode_title != "Title") %>% 
  mutate(episode = row_number())

tng_ep_names
```

Now join these episode names to the stardate dataframe.
...
stardate_tidy_names <- left_join(
  ...
  select(season, episode, episode_title, stardate)
...
```{r }
stardate_tidy_decimal <- stardate_tidy_names %>% 
  mutate(
